...
def getLogsCommand(update: Update, context):
    data = os.popen('cat /var/log/postgresql/postgresql.log | grep replication | tail -n 20').read()
    data = str(data).replace('\\n', '\n').replace('\\t', '\t')[2:-1]
...
def getPhoneChoice(update: Update, context):
    ...
    if user_input == '1':
        for i in range(len(phoneNumberList)):
           ...
           cursor.execute("INSERT INTO phones (phone) VALUES " + string + ";")
        connection.commit()
        update.message.reply_text('Запись прошла успешно')
        return ConversationHandler.END
    elif user_input == '2':
...
def main():
    ...
    dp.add_handler(convHandlerGetW)import logging
import re
import paramiko
import os
import psycopg2
...
def getLogsCommand(update: Update, context):
    data = os.popen('cat /var/log/postgresql/postgresql.log | tail -n 20').read()
    data = str(data).replace('\\n', '\n').replace('\\t', '\t')[2:-1]
    decoded_text = bytes(data, "utf-8").decode("unicode_escape").encode("latin1").decode("utf-8")
    update.message.reply_text(decoded_text)
    return ConversationHandler.END
